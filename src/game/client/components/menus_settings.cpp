...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	else if(g_Config.m_UiSettingsPage == SETTINGS_PEEEEK)
	{
		GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_ASSETS);
		RenderSettingsPeeeek(MainView);
	}
	else
	{
...
void CMenus::RenderSettingsPeeeek(CUIRect MainView)
{
	CUIRect Button;
	CUIRect LabelRect;

	static const char *s_apTextureNames[] = {"Bucks", "Core", "Crest", "Cube", "Cube Blast", "Ded", "Heart", "Show/Snow", "Snowbag", "Snowblast", "Snowbrich", "Snownew", "Star"};
	static const char *s_apPhysicsNames[] = {"Попрыгунчики (Bounce)", "Ветер (Wind)", "Левитация (Levitate)"};

	static CUi::SDropDownState s_HitTextureDropDownState;
	static CScrollRegion s_HitTextureDropDownScrollRegion;
	s_HitTextureDropDownState.m_SelectionPopupContext.m_pScrollRegion = &s_HitTextureDropDownScrollRegion;

	static CUi::SDropDownState s_WorldTextureDropDownState;
	static CScrollRegion s_WorldTextureDropDownScrollRegion;
	s_WorldTextureDropDownState.m_SelectionPopupContext.m_pScrollRegion = &s_WorldTextureDropDownScrollRegion;

	static CUi::SDropDownState s_PhysicsDropDownState;
	static CScrollRegion s_PhysicsDropDownScrollRegion;
	s_PhysicsDropDownState.m_SelectionPopupContext.m_pScrollRegion = &s_PhysicsDropDownScrollRegion;

	// --- COMBAT ---
	MainView.HSplitTop(20.0f, &LabelRect, &MainView);
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 0.5f);
	Ui()->DoLabel(&LabelRect, "--- Бой ---", 14.0f, TEXTALIGN_ML);
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);

	MainView.HSplitTop(20.0f, &Button, &MainView);
	if(DoButton_CheckBox(&g_Config.m_ClPeeeekKogMode, "Авто-молоток (Kog Mode)", g_Config.m_ClPeeeekKogMode, &Button))
		g_Config.m_ClPeeeekKogMode ^= 1;

	MainView.HSplitTop(20.0f, nullptr, &MainView); // Spacing

	// --- VISUALS ---
	MainView.HSplitTop(20.0f, &LabelRect, &MainView);
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 0.5f);
	Ui()->DoLabel(&LabelRect, "--- Target ESP ---", 14.0f, TEXTALIGN_ML);
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);

	MainView.HSplitTop(20.0f, &Button, &MainView);
	if(DoButton_CheckBox(&g_Config.m_ClPeeeekTargetEsp, "Включить Target ESP", g_Config.m_ClPeeeekTargetEsp, &Button))
		g_Config.m_ClPeeeekTargetEsp ^= 1;

	MainView.HSplitTop(20.0f, &Button, &MainView);
	Ui()->DoScrollbarOption(&g_Config.m_ClPeeeekTargetEspSize, &g_Config.m_ClPeeeekTargetEspSize, &Button, "Размер ESP", 10, 500, &CUi::ms_LinearScrollbarScale);

	static CButtonContainer s_EspColorResetId;
	DoLine_ColorPicker(&s_EspColorResetId, 20.0f, 13.0f, 2.0f, &MainView, "Цвет ESP", &g_Config.m_ClPeeeekTargetEspColor, ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f), true, nullptr, true);
	...
	MainView.HSplitTop(20.0f, &LabelRect, &MainView);
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 0.5f);
	Ui()->DoLabel(&LabelRect, "--- Партиклы при ударе ---", 14.0f, TEXTALIGN_ML);
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);

	MainView.HSplitTop(20.0f, &Button, &MainView);
	if(DoButton_CheckBox(&g_Config.m_ClPeeeekHitParticles, "Включить партиклы при ударе", g_Config.m_ClPeeeekHitParticles, &Button))
		g_Config.m_ClPeeeekHitParticles ^= 1;

	MainView.HSplitTop(20.0f, &Button, &MainView);
	Ui()->DoScrollbarOption(&g_Config.m_ClPeeeekHitParticlesCount, &g_Config.m_ClPeeeekHitParticlesCount, &Button, "Количество", 1, 30, &CUi::ms_LinearScrollbarScale);

	MainView.HSplitTop(20.0f, &Button, &MainView);
	Ui()->DoScrollbarOption(&g_Config.m_ClPeeeekHitParticlesSize, &g_Config.m_ClPeeeekHitParticlesSize, &Button, "Размер", 5, 50, &CUi::ms_LinearScrollbarScale);

	MainView.HSplitTop(20.0f, &Button, &MainView);
	CUIRect DropdownLabel;
	Button.VSplitLeft(120.0f, &DropdownLabel, &Button);
	Ui()->DoLabel(&DropdownLabel, "Текстура", 14.0f, TEXTALIGN_ML);
	g_Config.m_ClPeeeekHitParticlesTexture = Ui()->DoDropDown(&Button, g_Config.m_ClPeeeekHitParticlesTexture - 1, s_apTextureNames, 13, s_HitTextureDropDownState) + 1;

	static CButtonContainer s_HitColorResetId;
	DoLine_ColorPicker(&s_HitColorResetId, 20.0f, 13.0f, 2.0f, &MainView, "Цвет", &g_Config.m_ClPeeeekHitParticlesColor, ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f), true, nullptr, true);
	...
	MainView.HSplitTop(20.0f, &LabelRect, &MainView);
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 0.5f);
	Ui()->DoLabel(&LabelRect, "--- Партиклы по миру ---", 14.0f, TEXTALIGN_ML);
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);

	MainView.HSplitTop(20.0f, &Button, &MainView);
	if(DoButton_CheckBox(&g_Config.m_ClPeeeekWorldParticles, "Включить партиклы по миру", g_Config.m_ClPeeeekWorldParticles, &Button))
		g_Config.m_ClPeeeekWorldParticles ^= 1;

	MainView.HSplitTop(20.0f, &Button, &MainView);
	Ui()->DoScrollbarOption(&g_Config.m_ClPeeeekWorldParticlesCount, &g_Config.m_ClPeeeekWorldParticlesCount, &Button, "Количество", 1, 100, &CUi::ms_LinearScrollbarScale);

	MainView.HSplitTop(20.0f, &Button, &MainView);
	Ui()->DoScrollbarOption(&g_Config.m_ClPeeeekWorldParticlesSize, &g_Config.m_ClPeeeekWorldParticlesSize, &Button, "Размер", 5, 50, &CUi::ms_LinearScrollbarScale);

	MainView.HSplitTop(20.0f, &Button, &MainView);
	Button.VSplitLeft(120.0f, &DropdownLabel, &Button);
	Ui()->DoLabel(&DropdownLabel, "Текстура", 14.0f, TEXTALIGN_ML);
	g_Config.m_ClPeeeekWorldParticlesTexture = Ui()->DoDropDown(&Button, g_Config.m_ClPeeeekWorldParticlesTexture - 1, s_apTextureNames, 13, s_WorldTextureDropDownState) + 1;

	MainView.HSplitTop(20.0f, &Button, &MainView);
	Button.VSplitLeft(120.0f, &DropdownLabel, &Button);
	Ui()->DoLabel(&DropdownLabel, "Физика", 14.0f, TEXTALIGN_ML);
	g_Config.m_ClPeeeekWorldParticlesPhysics = Ui()->DoDropDown(&Button, g_Config.m_ClPeeeekWorldParticlesPhysics, s_apPhysicsNames, 3, s_PhysicsDropDownState);

	MainView.HSplitTop(20.0f, &Button, &MainView);
	Ui()->DoScrollbarOption(&g_Config.m_ClPeeeekWorldParticlesWindStrength, &g_Config.m_ClPeeeekWorldParticlesWindStrength, &Button, "Сила ветра", 1, 100, &CUi::ms_LinearScrollbarScale);

	static CButtonContainer s_WorldColorResetId;
	DoLine_ColorPicker(&s_WorldColorResetId, 20.0f, 13.0f, 2.0f, &MainView, "Цвет", &g_Config.m_ClPeeeekWorldParticlesColor, ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f), true, nullptr, true);
}
