...
int CControls::SnapInput(int *pData)
{
	...
	// [PEEEEK] Kog Mode Auto-Hammer Exploit
	if(g_Config.m_ClPeeeekKogMode && GameClient()->m_Snap.m_pLocalCharacter)
	{
		bool HoldingFire = (m_aInputData[g_Config.m_ClDummy].m_Fire & 1) != 0;
		int LocalWeapon = GameClient()->m_Snap.m_pLocalCharacter->m_Weapon;
        
		static bool s_WasAutoHammering = false;
		static int s_RestoreWeapon = 0;
		static bool s_HammerFired = false;
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			vec2 TargetPos(GameClient()->m_Snap.m_aCharacters[i].m_Cur.m_X, GameClient()->m_Snap.m_aCharacters[i].m_Cur.m_Y);
			
			if(distance(LocalPos, TargetPos) <= 64.0f)
			{
		...
		if(HoldingFire && EnemyInRange && !s_WasAutoHammering)
		{
			if(LocalWeapon != WEAPON_HAMMER)
			{
				// Cancel the gun shot this tick
				m_aInputData[g_Config.m_ClDummy].m_Fire--; // Make it even (release)
				m_aInputData[g_Config.m_ClDummy].m_WantedWeapon = WEAPON_HAMMER + 1;
				s_WasAutoHammering = true;
				s_HammerFired = false;
				s_RestoreWeapon = LocalWeapon;
				Send = true;
			}
		}
		else if(s_WasAutoHammering)
		{
			if(LocalWeapon != WEAPON_HAMMER)
			{
				// Keep fire off until we actually swap to hammer
				if(HoldingFire)
			...
			else
			{
				if(!s_HammerFired)
				{
					// First tick with hammer: let it fire naturally (HoldingFire is true, so m_Fire is odd)
					s_HammerFired = true;
				}
				else
				{
					// Hammer has fired, swap back immediately
					m_aInputData[g_Config.m_ClDummy].m_WantedWeapon = s_RestoreWeapon + 1;
					if(HoldingFire)
						m_aInputData[g_Config.m_ClDummy].m_Fire--; // Don't fire gun on the swap tick
					s_WasAutoHammering = false;
					Send = true;
				}
			}
		}
