...
void CPlayers::OnRender()
{
	...
	// [PEEEEK] Target ESP
	if(g_Config.m_ClPeeeekTargetEsp && GameClient()->m_Snap.m_pLocalCharacter)
	{
		int BestTargetId = -1;
		static int s_PeeeekLockedTargetId = -1;

		bool bIsHoldingHook = (GameClient()->m_Controls.m_aInputData[g_Config.m_ClDummy].m_Hook & 1) != 0;
		bool bIsHoldingFire = (GameClient()->m_Controls.m_aInputData[g_Config.m_ClDummy].m_Fire & 1) != 0;
		bool bIsHoldingLock = bIsHoldingHook || bIsHoldingFire;

		if(!bIsHoldingLock)
		{
			s_PeeeekLockedTargetId = -1;
		}
		else
		{
			int HookedPlayer = GameClient()->m_Snap.m_pLocalCharacter->m_HookedPlayer;
			int HookState = GameClient()->m_Snap.m_pLocalCharacter->m_HookState;
			if(HookState == 5 && HookedPlayer >= 0 && HookedPlayer < MAX_CLIENTS && IsPlayerInfoAvailable(HookedPlayer))
			{
				s_PeeeekLockedTargetId = HookedPlayer;
			}
		}

		vec2 LocalPos = GameClient()->m_LocalCharacterPos;
		// m_aTargetPos is an absolute world position (LocalPos + MouseOffset)
		vec2 CrosshairWorldPos = GameClient()->m_Controls.m_aTargetPos[g_Config.m_ClDummy];
		// Direction the player is aiming
		vec2 AimDir = normalize(CrosshairWorldPos - LocalPos);

		if(s_PeeeekLockedTargetId != -1 && IsPlayerInfoAvailable(s_PeeeekLockedTargetId))
		{
			BestTargetId = s_PeeeekLockedTargetId;
		}
		else
		{
			...
			for(int i = 0; i < MAX_CLIENTS; i++)
			{
				if(i == GameClient()->m_Snap.m_LocalClientId || !IsPlayerInfoAvailable(i))
					continue;

				vec2 TargetPos = GameClient()->m_aClients[i].m_RenderPos;
				vec2 DirToTarget = normalize(TargetPos - LocalPos);

				// Distance check (hook length)
				float DistToTarget = distance(LocalPos, TargetPos);
				if(DistToTarget > 380.0f)
					continue;

				// Line of sight check (can we hook them?)
				if(Collision()->IntersectLine(LocalPos, TargetPos, nullptr, nullptr))
					continue;

				// FOV check: 50 degree cone around aim direction
				float DotProduct = std::clamp(dot(AimDir, DirToTarget), -1.0f, 1.0f);
				float AngleToTarget = acos(DotProduct) * 180.0f / pi;

				if(AngleToTarget > 25.0f) // 50 degrees / 2 = 25 degree half-angle
					continue;

				// Score: prefer players closer to crosshair
				float DistToCrosshair = distance(CrosshairWorldPos, TargetPos);
			...
			if(bIsHoldingLock && BestTargetId != -1)
			{
				s_PeeeekLockedTargetId = BestTargetId;
			}
		}

		if(BestTargetId != -1)
		{
			vec2 TargetPos = GameClient()->m_aClients[BestTargetId].m_RenderPos;

			// Smooth fade at FOV edges
			vec2 DirToTarget = normalize(TargetPos - LocalPos);
			float DotProduct = std::clamp(dot(AimDir, DirToTarget), -1.0f, 1.0f);
			float AngleToTarget = acos(DotProduct) * 180.0f / pi;

			float FadeAlpha = 1.0f;
			if(BestTargetId != s_PeeeekLockedTargetId)
			{
				float FadeStart = 15.0f; // start fading at 15 degrees
				float FadeEnd = 25.0f;   // fully gone at 25 degrees
				if(AngleToTarget > FadeStart)
				{
					FadeAlpha = 1.0f - (AngleToTarget - FadeStart) / (FadeEnd - FadeStart);
					FadeAlpha = std::clamp(FadeAlpha, 0.0f, 1.0f);
				}
			}

			if(m_TargetEspTexture.IsValid())
			{
				float Size = (float)g_Config.m_ClPeeeekTargetEspSize;
				ColorRGBA EspColor = color_cast<ColorRGBA>(ColorHSLA(g_Config.m_ClPeeeekTargetEspColor, true));

				Graphics()->TextureSet(m_TargetEspTexture);
				Graphics()->BlendAdditive();
				Graphics()->QuadsBegin();

				float RotAngle = Client()->LocalTime() * 2.5f;
				Graphics()->QuadsSetRotation(RotAngle);
				Graphics()->SetColor(EspColor.r * FadeAlpha, EspColor.g * FadeAlpha, EspColor.b * FadeAlpha, 1.0f);
...
void CPlayers::OnInit()
{
	...
	// [PEEEEK] Target ESP textures - load as-is, additive blending handles black backgrounds
	m_TargetEspTexture = Graphics()->LoadTexture("peeeek/capture2.png", IStorage::TYPE_ALL);
}
